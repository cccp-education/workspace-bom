package cccp.vibecoding.contracts
...
class VibecodingContractsTest {
    ...
    @Test
    fun `CompositeContext toChannels returns 4 channels`() {
        val config = CompositeContextConfig()
        val ctx = CompositeContext("eager", "rag", "graphify", config)
        ...
        assertTrue(channels[3] is ContextChannel.Resource)
    }
    ...
    fun `CompositeContext channelsWithBudget truncates content`() {
        ...
        val ctx = CompositeContext(longContent, longContent, longContent, config)
        val budget = ChannelBudget()
        val budgeted = ctx.channelsWithBudget(budget)
        // Each channel should be truncated to fit its budget fraction
        assertTrue(budgeted[0].content.length < longContent.length || budgeted[0].content == longContent)
        assertEquals(4, budgeted.size)
    }
    ...
    fun `CompositeContextConfig rejects invalid budget`() {
        assertThrows<IllegalArgumentException> {
            CompositeContextConfig(budgetEagerLazy = 0.5, budgetRag = 0.5, budgetGraphify = 0.5, budgetOverhead = 0.5)
        }
    }
    ...
    fun `ChannelBudget default distribution`() {
        val budget = ChannelBudget()
        assertEquals(0.40, budget.eager, 0.001)
        assertEquals(0.30, budget.rag, 0.001)
        assertEquals(0.20, budget.graphify, 0.001)
        assertEquals(0.10, budget.resource, 0.001)
    }

    @Test
    fun `ChannelBudget rejects invalid sum`() {
        assertThrows<IllegalArgumentException> {
            ChannelBudget(eager = 1.0, rag = 0.5, graphify = 0.0, resource = 0.0)
        }
    }
